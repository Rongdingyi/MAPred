...
def get_dist_map(ec_id_dict, esm_emb, device, dtype, dot=False):
    """
...
def get_dist_map_rnn(ec_id_dict, esm_emb, device, dtype, model=None, dot=False):
    """
    Get the distance map for training, size of (N_EC_train, N_EC_train)
    between all possible pairs of EC cluster centers
    """
    ...
    if model is not None:
        model_emb = model(esm_emb.unsqueeze(1).to(device=device, dtype=dtype))
...
def get_dist_map_test(
    ...
    print(
        f"Calculating eval distance map, between {len(ids)} test ids "
