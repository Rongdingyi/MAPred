...
def write_pvalue_choices(df, csv_name, random_nk_dist_map, p_value=1e-5):
    out_file = open(csv_name, 'w', newline='')
    csvwriter = csv.writer(out_file, delimiter=',')
    all_test_EC = set()
...
def get_true_labels(file_name):
    ...
    return true_label_dict, all_label
...
def get_pred_labels(out_filename, pred_type="_maxsep"):
    ...
    for row in csvreader:
        ...
        pred_label_dict[row[0]] = preds_ec_lst
...
def get_eval_metrics(pred_label, true_label, all_label, average='weighted'):
    ...
    for i, id in enumerate(true_label.keys()):
        pred = pred_label[id] # use '0.0.0.0' as a special label for no prediction
        true = true_label[id]
        pred_m[i] = mlb.transform([pred])
        true_m[i] = mlb.transform([true])
    pre = precision_score(true_m, pred_m, average=average, zero_division=0)
    rec = recall_score(true_m, pred_m, average=average)
    f1 = f1_score(true_m, pred_m, average=average)
